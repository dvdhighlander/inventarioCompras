...
public class InventoryServiceImpl implements InventoryService {
	...
	@Retryable
	@Transactional(timeout = 30)
	public CompraResponse buyProduct(CompraDTO compra) {
		CompraResponse response = new CompraResponse(); 
		try {
		Inventory buyInventory = inventoryRepository.findById(compra.productoId()).orElseThrow();

		double disponible=buyInventory.getCantidad() - compra.cantidad();
      	if(disponible<0) {
    		throw new IllegalArgumentException();
    	}
		buyInventory.setCantidad(buyInventory.getCantidad()-compra.cantidad());
		inventoryRepository.save(buyInventory);
		response.setProducto_id(buyInventory.getProducto_id());
		response.setCantidadCompra(compra.cantidad());
		ProductClient clientProduct = new ProductClient(WebClient.builder());
		Mono<Products> rta = clientProduct.findProductbyId(buyInventory.getProducto_id());
		response.setResponseMessage(InventoryConstants.OK_BUY);
		response.setValorTotalCompra(compra.cantidad()*rta.block().getPrecio());
		System.out.println(InventoryConstants.OK_BUY);
		} 
		catch (NoSuchElementException ex) {
			throw new IllegalArgumentException(InventoryConstants.NO_PRODUCT);
		}
		catch (IllegalArgumentException ex) {
			throw new IllegalArgumentException(InventoryConstants.ERROR_NOT_ENOUGH_INVENTORY);
		}
		catch (Exception e) {
			response.setResponseMessage(InventoryConstants.ERROR_BUY + e.getMessage());
			System.out.println(InventoryConstants.ERROR_BUY + e.getMessage());
		}
